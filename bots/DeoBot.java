...
public class DeoBot extends Bot {
  ...
  //comparing manhattan distances of bullets to me
  int closestDistance;
  double newDistance;
  ...
  /** LOGIC
   * -increase counters
   * -search through liveBot list to see if aligned
   *    -fire at aligned bots
   * -find closest bullet index
   *    -if dist less than 50 and aligned
   *    -dodge, moving in calculated direction
   * -check if stuck,reverse direction if true
   * -randomly move R/D or L/U, depending on direction
   ...
   */
    ...
    * -tasks run every call - probably fine
    * -moves along set path (diagonal, topLeft -> bottomRight)
    * -shoots bullets at deadbots
    * -could use more returns to avoid running redundant lines
    */
